...
public class SeniorityResource {
     ...
     */
    @GetMapping("")
    public List<Seniority> getAllSeniorities(@RequestParam(name = "filter", required = false) String filter) {
        if ("ogloszenie-is-null".equals(filter)) {
            log.debug("REST request to get all Senioritys where ogloszenie is null");
            return StreamSupport
                .stream(seniorityRepository.findAll().spliterator(), false)
                .filter(seniority -> seniority.getOgloszenie() == null)
                .toList();
        }
