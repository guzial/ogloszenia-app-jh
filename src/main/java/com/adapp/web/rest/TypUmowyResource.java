...
public class TypUmowyResource {
     ...
     */
    @GetMapping("")
    public List<TypUmowy> getAllTypUmowies(@RequestParam(name = "filter", required = false) String filter) {
        if ("ogloszenie-is-null".equals(filter)) {
            log.debug("REST request to get all TypUmowys where ogloszenie is null");
            return StreamSupport
                .stream(typUmowyRepository.findAll().spliterator(), false)
                .filter(typUmowy -> typUmowy.getOgloszenie() == null)
                .toList();
        }
